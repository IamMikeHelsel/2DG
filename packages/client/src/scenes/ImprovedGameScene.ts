...
export class ImprovedGameScene extends Phaser.Scene {
  ...
  private mobs = new Map<
    string,
    { body: Phaser.GameObjects.Container; hp: Phaser.GameObjects.Rectangle }
  >();
  ...
  async create() {
      ...
      if (isLocal) {
        ...
        this.add
          .text(
            ...
            "Controls: Arrow keys or Right-click to move | SPACE to attack | E for shop | ENTER to chat",
            {
              color: "#aaaaaa",
              fontSize: "12px",
              stroke: "#000000",
    ...
    this.room.state.players.onChange = (p: ServerPlayer, key: string) => {
      ...
      if (isLocal) {
        ...
        if (
          Math.abs(this.predictedPosition.x - p.x) > threshold ||
