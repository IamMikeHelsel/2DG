export class SimpleSpriteGen {
  static createColoredRect(scene: Phaser.Scene, x: number, y: number, size: number, color: number, name?: string) {
  ...
  static createNPC(scene: Phaser.Scene, x: number, y: number, size: number, color: number, name: string) {
    const container = scene.add.container(x, y);
